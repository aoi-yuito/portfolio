...
onNuxtReady(() => {
  ...
  const canvas = document.querySelector(".scattered-text");
  ...
  const getTextCanvas = () => {
    ...
    textCtx.font = `bold ${config.font * window.devicePixelRatio}px Arial`;
  ...
  // ============== uncomment if you uncomment the tweakpane =================
  //const sync = (event) => {
  //  if (
  ...
  // ============= uncomment from below if needed =================
...
<template>
  <canvas class="scattered-text"></canvas>
</template>
