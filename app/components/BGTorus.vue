...
onNuxtReady(() => {
  ...
  function createSmallToruses() {
    ...
    for (let i = 0; i < params.numSmallToruses; i++) {
      const geometry = new THREE.TorusGeometry(
        params.smallTorusSize,
        params.smallTorusTube,
        16,
        100,
      );
      const material = new THREE.MeshStandardMaterial({
  ...
  function setupTweakpane() {
    ...
    motionFolder.addBinding(params, "speed", {
      min: -0.05,
      max: 0.05,
      step: 0.001,
      label: "Big Torus Speed",
    });
    motionFolder.addBinding(params, "smallTorusSpeed", {
      min: -0.05,
      max: 0.05,
      step: 0.001,
      label: "Small Torus Speed",
    });
    motionFolder.addBinding(params, "sphereSpeed", {
      min: -0.05,
      max: 0.05,
      step: 0.001,
      label: "Sphere Speed",
    });
    motionFolder.addBinding(params, "radius", {
      min: 2,
      max: 20,
      step: 0.5,
      label: "Path Radius",
    ...
    cameraFolder
      .addBinding(params, "cameraX", { min: -30, max: 30, step: 0.5 })
      .on("change", updateCameraPosition);
    cameraFolder
      .addBinding(params, "cameraY", { min: -30, max: 30, step: 0.5 })
      .on("change", updateCameraPosition);
    cameraFolder
      .addBinding(params, "cameraZ", { min: 5, max: 40, step: 0.5 })
      .on("change", updateCameraPosition);

    // Center settings
    const centerFolder = pane.addFolder({ title: "Center Position" });
    centerFolder.addBinding(params, "centerX", {
      min: -20,
      max: 20,
      step: 0.5,
      label: "Center X",
    });
    centerFolder.addBinding(params, "centerY", {
      min: -20,
      max: 20,
      step: 0.5,
      label: "Center Y",
    });
    centerFolder.addBinding(params, "centerZ", {
      min: -20,
      max: 20,
      step: 0.5,
      label: "Center Z",
    ...
    objectFolder
      .addBinding(params, "numSmallToruses", {
        min: 1,
        max: 30,
        step: 1,
        label: "Small Toruses",
    ...
    objectFolder
      .addBinding(params, "torusRadius", {
        min: 0.3,
        max: 3,
        step: 0.1,
        label: "Big Torus Size",
      })
      .on("change", createToruses);
    objectFolder
      .addBinding(params, "torusTube", {
        min: 0.05,
        max: 0.5,
        step: 0.05,
        label: "Big Torus Tube",
      })
      .on("change", createToruses);
    objectFolder
      .addBinding(params, "smallTorusSize", {
        min: 0.2,
        max: 2,
        step: 0.1,
        label: "Small Torus Size",
      })
      .on("change", createSmallToruses);
    objectFolder
      .addBinding(params, "smallTorusTube", {
        min: 0.05,
        max: 0.3,
        step: 0.05,
        label: "Small Torus Tube",
      })
      .on("change", createSmallToruses);
    objectFolder
      .addBinding(params, "sphereSize", {
        min: 0.1,
        max: 1.5,
        step: 0.1,
        label: "Sphere Size",
      ...
      .on("change", createSpheres);
    objectFolder.addBinding(params, "objectScale", {
      min: 0.1,
      max: 2,
      step: 0.1,
    });
  ...
  function animate() {
    ...
    toruses.forEach((torusObj) => {
      ...
      const x =
        Math.sin(torusObj.angle) * params.radius * Math.cos(tiltRad) +
        params.centerX;
      const y = Math.cos(torusObj.angle) * params.radius + params.centerY;
      const z =
        Math.sin(torusObj.angle) * params.radius * Math.sin(tiltRad) +
        params.centerZ;

      torusObj.mesh.position.set(x, y, z);

      const tangentX =
        Math.cos(torusObj.angle) * params.radius * Math.cos(tiltRad);
      const tangentY = -Math.sin(torusObj.angle) * params.radius;
      const tangentZ =
        Math.cos(torusObj.angle) * params.radius * Math.sin(tiltRad);
      ...
      const scale =
        (1 - ((distance - minDistance) / (maxDistance - minDistance)) * 0.3) *
        params.objectScale;
    ...
    spheres.forEach((sphereObj) => {
      ...
      const x =
        Math.sin(sphereObj.angle) * params.radius * Math.cos(tiltRad) +
        params.centerX;
      const y = Math.cos(sphereObj.angle) * params.radius + params.centerY;
      const z =
        Math.sin(sphereObj.angle) * params.radius * Math.sin(tiltRad) +
        params.centerZ;
